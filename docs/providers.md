...
Pi-compatible forks should be custom providers that extend `pi`, override `command`, and set `params.sessionDir` to their JSONL session directory. That keeps fork-specific product identity in config instead of adding a first-class provider for every fork, while still allowing terminal-started sessions to appear in import flows.
